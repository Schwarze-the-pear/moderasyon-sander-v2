...
module.exports.run = (client, message, args) => {
  ...
  let tag = "ꁚ";
  const booster = message.guild.roles.cache.get("710458789836619797").members.size;
  const kız = message.guild.roles.cache.get("752178345202024458").member.size;
  const erkek = message.guild.roles.cache.get("752178449291935834").member.size;
  const online = message.guild.members.cache.filter(
    u => u.presence.status != "offline"
  ).size;
  const ttag = message.guild.members.cache.filter(m =>
  ...
  message.channel.send(
    embed.setDescription(`**Toplam Üye  ・ ${toplam}
  Aktif Üye ・ ${online}
  Taglı Üye ・ ${ttag}
  Booster Üye ・${booster}
  Kız Üye ・ ${kız}
